use crate::{
    gltf_nameless_name_generator, vertex_attributes::convert_attribute, Gltf, GltfAssetLabel, GltfExtras, GltfMaterialExtras, GltfMaterialName, GltfMeshExtras, GltfNode, GltfSceneExtras, GltfSkin
};
    ...
    let (animations, named_animations, animation_roots) = {
        ...
        for animation in gltf.animations() {
            ...
            let handle = load_context.add_labeled_asset(
                GltfAssetLabel::Animation(animation.name().and_then(|n| Some(n.to_owned())).unwrap_or(format!("#{}", animation.index())).into()).to_string(),
                animation_clip,
            );
    ...
    for gltf_mesh in gltf.meshes() {
        ...
        for primitive in gltf_mesh.primitives() {
            let primitive_label = GltfAssetLabel::Primitive {
                mesh: gltf_nameless_name_generator(gltf_mesh.name(), gltf_mesh.index(), "Mesh").into(),
    ...
    match gltf_texture.source().source() {
        Source::View { view, mime_type } => {
            ...
            Ok(ImageOrPath::Image {
                image,
                label: GltfAssetLabel::Texture(gltf_nameless_name_generator(gltf_texture.name(), gltf_texture.index(), "Texture").into()),
            })
        }
        ...
        if !settings.load_meshes.is_empty() {
            if let Some(mesh) = gltf_node.mesh() {
                ...
                for primitive in mesh.primitives() {
                    ...
                    let primitive_label = GltfAssetLabel::Primitive {
                        mesh: gltf_nameless_name_generator(mesh.name(), mesh.index(), "Mesh").into(),
                        primitive: primitive.index(),
                    };
...
fn texture_handle(load_context: &mut LoadContext, texture: &gltf::Texture) -> Handle<Image> {
    let tex = GltfAssetLabel::Texture(gltf_nameless_name_generator(texture.name(), texture.index(), "Texture").into());
...
fn scene_label(scene: &gltf::Scene) -> String {
    GltfAssetLabel::Scene(scene.name().and_then(|n| Some(n.to_owned())).unwrap_or(format!("#{}", scene.index())).into()).to_string()
}

/// Return the label for the `skin`.
fn skin_label(skin: &gltf::Skin) -> String {
    GltfAssetLabel::Skin(gltf_nameless_name_generator(skin.name(), skin.index(), "Skin").into()).to_string()
}

/// Return the label for the `inverseBindMatrices` of the node.
fn inverse_bind_matrices_label(skin: &gltf::Skin) -> String {
    GltfAssetLabel::InverseBindMatrices(gltf_nameless_name_generator(skin.name(), skin.index(), "Skin").into()).to_string()
}
...
"#,
        ...
        assert_eq!(gltf_node.asset_label(), GltfAssetLabel::Node("TestSingleNode".into()));
    }
